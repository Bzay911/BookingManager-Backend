      ...
      switch (event.type) {
        case "checkout.session.completed": {
          ...
          if (type === "BOOKING") {
            ...
            const nextPosition = lastQueueEntry
            ...
            const isBookingToday =
              queueEntry.booking.scheduledAt >= todayStart &&
              queueEntry.booking.scheduledAt <= todayEnd;

            // Emit socket event only if booking is for today
            if (isBookingToday) {
              const io = getIO();
              io.to(`business:${booking.businessId}`).emit(
                "queue:user-added",
                queueEntry,
              );
              console.log(
                `Emitted queue:user-added to room business:${booking.businessId}`,
              );
            }
